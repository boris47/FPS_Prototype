        ...
        private void UpdateWind()
        {
            if (EnableWind && WindZone != null && WindSpeedRange.y > 1.0f)
            {
                ...
                if (nextWindTime < Time.time)
                {
                    ...
                    audioSourceWind.Play((WindZone.windMain / WindSpeedRange.z) * WindSoundVolumeModifier * RainIntensity);

//					ParticleSystem.ExternalForcesModule e = RainFallParticleSystem.externalForces;
//					e.multiplier = RainIntensity * RainIntensity;
                }
            }
            else
            {
