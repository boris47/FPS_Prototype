...
public class PathSpline : PathBase
{
	...
	public 	override void	IteratePath(System.Action<PathWayPointOnline> OnPosition)
	{
		...
		{
			System.Array.ForEach(m_Waypoints, wayPoint => OnPosition( wayPoint ));
		}
	}
	...
	public	override	bool	Move(Transform subject, float? speed, Vector3? upwards)
	{
		...
		// Start event
		if (!m_IsStartEventCalled && m_Interpolant == 0.0f)
		{
			if (m_OnPathStart.IsNotNull() && m_OnPathStart.GetPersistentEventCount() > 0)
			{
		...
		// Interpolant
		m_Interpolant += speed ?? m_Speed;



		// End event
		if (Mathf.Abs(m_Interpolant) >= 1.0f && !m_IsCompleted)
		{
			if (!m_IsCompleted)
			{
				m_IsCompleted = true;

				if (!m_IsEndEventCalled && m_OnPathCompleted.IsNotNull() && m_OnPathCompleted.GetPersistentEventCount() > 0)
				{
