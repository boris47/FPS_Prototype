...
//	protected		AnimationClip				m_FireAnim						= null;
	...
	protected	virtual		void			Awake()
	{
		...
		// Animations
		{
			bIsInitilalizedSuccessfully &= Utils.Base.SearchComponent(gameObject, out m_Animator, ESearchContext.LOCAL );
...
//		this.m_AreAttachmentsAllowed = this.transform.SearchComponentInChild( "Attachments", ref this.m_AttachmentRoot );
		...
		bIsInitilalizedSuccessfully &= bIsInitilalizedSuccessfully && GlobalManager.Configs.GetSection( m_WpnBaseSectionName, ref m_WpnSection );
...
//		m_WpnFallFeedback = Vector3.ClampMagnitude( m_WpnCurrentDeviation, WPN_FALL_FEEDBACK_CLAMP_VALUE );
	...
	public					void			Hide()
	{
		transform.SearchComponents( ref m_WeaponRenderes, ESearchContext.CHILDREN, (r) => r.enabled == true );
