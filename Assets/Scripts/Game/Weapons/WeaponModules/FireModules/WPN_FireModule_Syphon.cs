	...
	{	
		...
		if ( moduleSection.bAsString( "Module_Prefab", ref modulePrefabPath ) )
		{
			...
			{	
				...
				if ( modulePrefabInstance.transform.SearchComponent( ref m_Laser, ESearchContext.LOCAL ) )
				{
					m_Laser.LaserLength = m_BeamLength;
					m_Laser.enabled = false;
				}

				if (m_WeaponRef.Transform.SearchComponent( ref m_Renderer, ESearchContext.CHILDREN, s => s.name == "Graphics" ) )
				{
...
//		m_AudioSourceFire.Play();
	...
	protected void FixedUpdate()
	{
		...
		if (m_Laser.enabled == true && m_Laser.HasHit == true )
		{
			...
			if ( Utils.Base.SearchComponent( m_Laser.RayCastHit.transform.gameObject, out IEntity entity, ESearchContext.LOCAL ) )
			{
