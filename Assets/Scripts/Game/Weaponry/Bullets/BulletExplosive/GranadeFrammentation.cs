...
public sealed class GranadeFrammentation : BulletExplosive, ITimedExplosive {
	...
	protected override	void	OnExplosion()
	{
		...
		for ( int i = 0; i < nresults; i++ )
		{
			...
			// Entity
			bool bIsEntity = Utils.Base.TrySearchComponent( hittedCollider.gameObject, ESearchContext.LOCAL, out IEntity entityInterface );
			bool bHasShield = Utils.Base.TrySearchComponent( hittedCollider.gameObject, ESearchContext.LOCAL, out IShield shield );
...
//				else
			...
			// Dynamic props
			Rigidbody rb = null;
			if ( bIsEntity == false && hittedCollider.transform.TrySearchComponent(ESearchContext.LOCAL, out rb ) )
			{
