...
public sealed class SurfaceManager : GameMonoBehaviourSingleton<SurfaceManager>
{
	[System.Serializable]
	...
	private		bool			TryGetSurfaceIndex( in Collider col, in Ray ray, out int index )
	{
		...
		{
			if (Physics.Raycast(ray, out RaycastHit hit) && hit.collider.gameObject.TrySearchComponent(ESearchContext.LOCAL, out Renderer renderer))
			{
