...
namespace Entities.AI.Components.Behaviours
{
	internal class BehaviourTreeView : GraphView
	{
		...
		{
			foreach(System.Type derivedType in TypeCache.GetTypesDerivedFrom<BTTaskNode>())
			{
			...
			foreach (System.Type derivedType in TypeCache.GetTypesDerivedFrom<BTCompositeNode>())
			{
				string name = BehaviourTreeEditorUtils.GetInstancePropertyValue<string>(derivedType, "NodeName") ?? derivedType.Name;
				m_MappedTypes.Add($"Composites/{name}", derivedType);
			}

			foreach (System.Type derivedType in TypeCache.GetTypesDerivedFrom<BTDecoratorNode>())
			{
				string name = BehaviourTreeEditorUtils.GetInstancePropertyValue<string>(derivedType, "NodeName") ?? derivedType.Name;
		...
		//////////////////////////////////////////////////////////////////////////
		private NodeViewBase FindNodeView(IBTNodeEditorInterface node) => GetNodeByGuid(node.Guid) as NodeViewBase;
		...
		public void PopulateView(BehaviourTree InBehaviourTree)
		{
			...
			BTRootNode rootNode = m_BehaviourTree.AsEditorInterface.EnsureRootNode();
			{
				// Creates nodes views
				NodeViewBase rootNodeView = CreateNodeView(rootNode);
				...
				// Create edges
				{
					if (rootNode.Child)
					{
						NodeViewBase childNodeView = FindNodeView(rootNode.Child);
						AddElement(rootNodeView.ConnectTo(childNodeView));
					}
		...
		private GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
		{
			...
			{
				nodes.ForEach(n => n.GetAsBTNodeView().SortChildren());
			}
			if (!IsBTInstance)
			{
		...
		private NodeViewBase CreateNodeView(in BTNode InNode, Vector2? OptPosition = null)
		{
			...
			if (EditorApplication.isPlaying)
			{
				...
				InNode.OnNodeActivation += BreakpointCheck;
		//
				InNode.OnNodeUpdate -= BreakpointCheck;
				InNode.OnNodeUpdate += BreakpointCheck;
		//
