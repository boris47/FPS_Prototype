...
namespace Entities.AI.Components.Behaviours
{
	internal class BehaviourTreeView : GraphView
	{
		...
		private GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
		{
			...
			if (graphViewChange.edgesToCreate.IsNotNull())
			{
				graphViewChange.edgesToCreate.ForEach(edge =>
				{
					NodeViewBase parentView = edge.output.node.GetAsBTNodeView();
					NodeViewBase childView = edge.input.node.GetAsBTNodeView();
					parentView.OnConnectionWith(edge, childView);
				});
				BehaviourTreeEditorUtils.AssignChildrenIndexes(m_BehaviourTree.AsEditorInterface.RootNode);
			}
		...
		public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
		{
			return ports.ToList()
			.Where(candidatePort =>
				candidatePort.direction != startPort.direction &&
				candidatePort.node != startPort.node &&
				(
					candidatePort.GetBTNodePort().PortTypeSelectors?.Any(s => ReflectionHelper.IsInerithedFrom(s, startPort.GetBTNode().GetType())) ?? true
				)
			)
			.ToList();
		}
