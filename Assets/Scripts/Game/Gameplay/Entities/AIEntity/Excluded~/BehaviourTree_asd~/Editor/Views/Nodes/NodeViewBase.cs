...
namespace Entities.AI.Components.Behaviours
{
	internal abstract class NodeViewBase : Node
	{
		...
		public NodeViewBase(in BTNode InNode, in EdgeConnectorListener InEdgeConnectorListener) : base(k_BehaviourTreeNodeViewPath)
		{
			...
			{
				Rect p = GetPosition();
				Vector2 storedPosition = BTNode.Editor.GetEditorGraphPosition(InNode);
				p.xMin = storedPosition.x;
				p.yMin = storedPosition.y;
				SetPosition(p);
			}
		...
		public void OnEdgeCreation(in Edge InEdge, in NodeViewBase InChild, in uint? InPortIndex = null)
		{
			...
			// Add node to parent
			BehaviourTree.Editor.AddChildTo(parentView.BehaviourTreeNode, childView.BehaviourTreeNode, InPortIndex);

			// Set parent output port to child parent port index
			uint portIndex = InPortIndex ?? InEdge.output.GetBTNodePort().PortIndex;
			BTNode.Editor.SetNodeParentPortIndex(InChild.BehaviourTreeNode, portIndex);
		...
		public void DisconnectFrom(in NodeViewBase InChild)
		{
			...
			uint portIndex = BTNode.Editor.GetNodeParentPortIndex(InChild.BehaviourTreeNode);
			...
			BTNode.Editor.SetNodeParentPortIndex(childNode, 0u);
		}
