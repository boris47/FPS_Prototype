...
namespace Entities.Player.Components
{
	...
	public class CustomCharacterController : MonoBehaviour
	{
		...
		/// </summary>
		public event System.Action<CollisionInfo> collision;
		...
		public struct CollisionInfo
		{
			...
			readonly CustomCharacterController m_Controller;
			...
			/// </summary>
			public CustomCharacterController controller { get { return m_Controller; } }
			...
			public CollisionInfo(CustomCharacterController openCharacterController,
								 RaycastHit hitInfo,
		...
		/*
		private const			int							k_MaxSweepSteps						= 5;
		private const			float						k_MinCeilingAngle					= 145f;
		...
		private void Awake()
		{
			...
			if (Utils.CustomAssertions.IsTrue(gameObject.TryGetComponent(out m_Collider)))
			{
				m_Collider.center = m_Center;
				m_Collider.height = m_Height;
				m_Collider.radius = m_Radius;

				if (m_Collider.material == null)
				{
					m_Collider.material = new PhysicMaterial("No Friction") { dynamicFriction = 0, staticFriction = 0, bounciness = 0 };
				}
			}
		}
		...
		//////////////////////////////////////////////////////////////////////////
		public void RequestLocalMove(in Vector3 InLocalSpaceMoveVector)
		{
			RequestMove(transform.TransformDirection(InLocalSpaceMoveVector));
		}

		//////////////////////////////////////////////////////////////////////////
		/// <summary>  </summary>
		/// <param name="InWorldSpaceMoveVector"></param>
		public void RequestMove(in Vector3 InWorldSpaceMoveVector)
		{
			// Get State
			m_Position.Set(m_Rigidbody.position);
			m_WorldVelocity.Set(InWorldSpaceMoveVector);
			...
			// Consume Input
			m_DisplacementVector.Set(Vector3.zero);// = Vector3.zero;
		}

		//////////////////////////////////////////////////////////////////////////
		public void RequestTeleport(in Vector3 InWorldLocation)
		{
			m_Rigidbody.interpolation = RigidbodyInterpolation.None;
			{
				m_Position.Set(InWorldLocation);
		...
		{
			Vector3 localVelocity = transform.InverseTransformDirection(m_WorldVelocity);
			...
			lateralVelocity.Set(transform.TransformDirection(lateralVelocity));
			verticalVelocity.Set(transform.TransformDirection(verticalVelocity));

			CapsuleSweep(lateralVelocity.normalized, lateralVelocity.magnitude, m_StepOffset, k_MinCeilingAngle);
		//	CapsuleSweep(verticalVelocity.normalized, verticalVelocity.magnitude, 0f, 0f, m_SlopeLimit);
		}
		...
		private void HandleContacts()
		{
			if (m_Contacts.Count > 0)
			{
				...
				foreach (RaycastHit contact in m_Contacts)
				{
					...
					m_WorldVelocity.Sub(Vector3.Project(m_WorldVelocity, contact.normal));
				}
			}
		}
		...
		private void Depenetrate()
		{
			...
			int overlapsNum = Physics.OverlapCapsuleNonAlloc(top, bottom, m_Collider.radius, m_Overlaps, Physics.AllLayers, QueryTriggerInteraction.Ignore);
			if (overlapsNum > 0)
			{
				for (int i = 0; i < overlapsNum; i++)
				{
					Transform otherTransform = m_Overlaps[i].transform;
					if ((m_Overlaps[i].transform != transform) &&
						Physics.ComputePenetration
						(
							m_Collider, m_Position, transform.rotation,
							m_Overlaps[i], otherTransform.position, otherTransform.rotation,
							out Vector3 direction, out float distance
						)
					)
					{
						m_Position.Add(direction * (distance + m_SkinWidth));
						m_WorldVelocity.Sub(Vector3.Project(m_WorldVelocity, -direction));
					}
				}
			}
		}
		...
		//////////////////////////////////////////////////////////////////////////
		private void CapsuleSweep(Vector3 InDirection, float InDistance, float InStepOffset, float InMinSlideAngle = 0f, float InMaxSlideAngle = 360f)
		{
			...
			for (int i = 0; i < k_MaxSweepSteps; i++)
			{
				origin = m_Position + m_Center - (InDirection * m_Radius);
				bottom = origin - (m_UpDirection * (capsuleOffset - InStepOffset));
				top = origin + (m_UpDirection * capsuleOffset);

				if (Physics.CapsuleCast(point1: top, point2: bottom, radius: m_Radius, direction: InDirection, hitInfo: out hitInfo, maxDistance: InDistance + m_Radius, layerMask: Physics.AllLayers, queryTriggerInteraction: QueryTriggerInteraction.Ignore))
				{
					slideAngle = Vector3.Angle(m_UpDirection, hitInfo.normal);
					safeDistance = hitInfo.distance - m_Radius - m_SkinWidth;
					m_Position.Add(InDirection * safeDistance);
					m_Contacts.Add(hitInfo);

					if ((slideAngle >= InMinSlideAngle) && (slideAngle <= InMaxSlideAngle))
					{
						break;
					}

					InDirection.Set(Vector3.ProjectOnPlane(InDirection, hitInfo.normal));
					InDistance -= safeDistance;
				}
				else
				{
					m_Position.Add(InDirection * InDistance);
					break;
				}
			}
		}
