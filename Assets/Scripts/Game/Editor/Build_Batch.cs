...
public class Build_Batch
{
	...
	[MenuItem(MENU_LABEL + "/Auto Refresh")]
	...
	[MenuItem(MENU_LABEL + "/Auto Refresh", true)]
	static bool AutoRefreshToggleValidation()
	{
		var status = EditorPrefs.GetInt("kAutoRefresh");
		if (status == 1)
			Menu.SetChecked(MENU_LABEL + "/Auto Refresh", true);
		else
			Menu.SetChecked(MENU_LABEL + "/Auto Refresh", false);
		return true;
	}
...
//	[MenuItem( MenuLabel + "/CreateBundle", priority = 3 )]
	...
	{
		// Search for build settings in corresponding section
		using (Database.LocalInstance buildSettings = new Database.LocalInstance())
		{
			if (buildSettings.TryLoadFile(BuildSettingsConfigFile))
			{
				if (buildSettings.TryGetSection(buildSettingsSectionName, out DatabaseCore.Section buildSettingsSection))
				{
